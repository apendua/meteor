...
_.extend(Slice.prototype, {
  ...
  build: function () {
    ...
    _.each(self.getSourcesFunc(), function (source) {
      ...
      var compileStep = {
        ...
        addJavaScript: function (options) {
          ...
          // TODO: layer can be only used on browser targets
          js.push({
            source: options.data,
            ...
            bare: !!options.bare,
            sourceMap: options.sourceMap,
            layer: (_.has(options, 'layer') ? pkgPrefix(options.layer) : pkgPrefix(compileStep.fileOptions.layer)),
          });
        },
    ...
    _.each(js, function (item) {
      if (item.layer) {
        // TODO: do we really want to use sliceSuffix of this form? is it amd complaint?
        layerServePaths[item.layer] = isApp ?
    ...
    var results = linker.prelink({
      ...
      layerServePaths: layerServePaths,
        ...
        sources = _.map(sources, function (relPath) {
          ...
          if (sliceName === "client") {
            ...
            if ((path.sep + relPath).indexOf(clientLayersSubstr) !== -1) {
              // TODO: probably, we should add some prefix to the module name
